  ...
  useEffect(() => {
	...
	const sidebarItems = [
		...
		},
		{
			...
			icon: '🚙',
			roles: ['Park Manager', 'Booking Staff', 'Guide'],
		},
		{
			id: 'vehicles',
			label: 'Vehicle Management',
			icon: '🔧',
			roles: ['Park Manager', 'Maintenance Staff'],
		},
		{
			id: 'staff',
			label: 'Staff Management',
			icon: '👥',
			roles: ['Park Manager', 'Supervisor'],
		},
		{
			id: 'checkin',
			label: 'Check-In System',
			icon: '✅',
			roles: ['Booking Staff', 'Guide'],
		},
		{
			id: 'operations',
			label: 'Park Operations',
			icon: '🏞️',
			roles: ['Park Manager', 'Park Ranger'],
		},
		{
			id: 'analytics',
			label: 'Analytics & Reports',
			icon: '📈',
			roles: ['Park Manager', 'Admin'],
		},
		{
			id: 'communication',
			label: 'Communication',
	...
	const renderContent = () => {
		switch (activeTab) {
			...
			case 'safaris':
				return (
					<SafariManagement
						safaris={employeeDashboardData.todaysSafaris}
					/>
				);
			case 'vehicles':
				return (
					<VehicleManagement
						vehicles={employeeDashboardData.vehicles}
					/>
				);
			case 'staff':
			...
			case 'operations':
				return (
					<ParkOperations
						operations={employeeDashboardData.parkOperations}
					/>
				);
			case 'analytics':
				return (
					<AnalyticsReports
						analytics={employeeDashboardData.analytics}
					/>
				);
			case 'communication':
				return (
					<CommunicationCenter
						communication={employeeDashboardData.communication}
					/>
				);
			default:
				return <DashboardOverview data={employeeDashboardData} />;
